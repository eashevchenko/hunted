...
public class BulletWorld extends BaseWorld<BulletEntity> {
	...
	@Override
	public void render(String tag, ModelBatch batch, Environment lights, Iterable<BulletEntity> entities) {
		if (renderMeshes)	super.render(tag, batch, lights, entities);
		if (debugDrawer != null && debugDrawer.getDebugMode() > 0) {
			batch.flush();
			debugDrawer.begin(batch.getCamera());
			collisionWorld.debugDrawWorld();
